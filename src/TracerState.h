...
class TracerState {
  ...
  public:
    TracerState(const std::string &output_dirpath,
        ...
        : output_dirpath_(output_dirpath), trace_filepath_(trace_filepath),
          ...
          environment_id_(0), variable_id_(0), timestamp_(0),
          call_id_counter_(0), denoted_value_id_counter_(0),
          object_count_(OBJECT_TYPE_TABLE_COUNT, 0) {

        object_count_data_table_ = create_data_table(
            output_dirpath_ + "/" + "object-count", {"type", "count"},
        ...
        call_summary_data_table_ = create_data_table(
            ...
            {"function_id", "function_type", "formal_parameter_count", "wrapper",
             ...
             "return_value_type", "call_count"},
            truncate_, binary_, compression_level_);

        function_definition_data_table_ =
            create_data_table(output_dirpath_ + "/" + "function-definition",
        ...
        free_promise_data_table_ =
            create_data_table(output_dirpath_ + "/" + "free-promises",
                              {"value_id",
                               "type",
                               "expression_type",
                               "value_type",
    ...
    void initialize() { serialize_configuration_(); }
    ...
    void increment_object_count(sexptype_t type) { ++object_count_[type]; }
  ...
  private:
    ...
    void serialize_free_promise_(DenotedValue *promise) {
        free_promise_data_table_->write_row(
            ...
            promise->get_force_count(), promise->get_value_lookup_count(),
  ...
  private:
    timestamp_t get_current_timestamp_() const { return timestamp_; }

    timestamp_t increment_timestamp_() { return timestamp_++; }
  ...
  public:
    Call *create_call(const SEXP call, const SEXP op, const SEXP args,
        ...
        function_call =
            new Call(call_id, function_id, TYPEOF(op), function_name,
                     function->get_formal_parameter_count(), rho, function);
  ...
  private:
    call_id_t get_next_call_id_() { return ++call_id_counter_; }
                ...
                case DOTSXP:
                    for (SEXP dot_dot_arguments = argument;
                        ...
                        process_closure_argument_(
                            ...
                            actual_argument_position, name, dot_dot_argument);
                    }
                    break;
        ...
        for (DenotedValue *argument : call->get_arguments()) {
            argument_data_table_->write_row(
                ...
                argument->get_evaluation_depth()
                    .forcing_actual_argument_position,
                argument->get_force_count(), argument->get_value_lookup_count(),
    ...
    void serialize_function_call_summary_(const Function &function) {
        ...
        for (std::size_t i = 0; i < function.get_summary_count(); ++i) {
            call_summary_data_table_->write_row(
                ...
                function.get_formal_parameter_count(), function.is_wrapper(),
                all_names,
                function.get_generic_method_name(), function.is_dispatcher(),
    ...
    void serialize_function_definition_(const Function &function) {
        ...
        if (res.second) {
            function_definition_data_table_->write_row(
                function.get_id(), function.is_byte_compiled(),
    ...
    void update_wrapper_state(Call *call) {
        ...
        if (!stack.is_empty()) {
            ...
            if (exec_ctxt.is_closure()) {
                ...
                Function *caller_function = exec_ctxt.get_closure() -> get_function();
                caller_function->update_wrapper(called_function->is_internal() ||
